...
namespace LinqExtensionMethods {
    class Program {
        ...
        Pricing[] priceList = {
            ...
            var ProductWithPrices = from prod in productList
            ...
            foreach(var p in ProductWithPrices) {
            ...
            var total3 = nbrs2.Where(nbr => nbr % 3 == 0).Sum();
            Console.WriteLine($"Total Divisible by 3: {total3}");
            ...
            var nbrsSumDivBy3Ext = /*(*/from n in nbrs2
                                   where n % 3 == 0
                                   select n/*).Sum(); to sum after*/;  //declares what is going to happen, but does not execute until variable is called
